...
def get_player_info(uid):
    try:
        url = f"https://flash-by-info.vercel.app/info?uid={uid}&key=Flash"
...
def load_credentials_from_file(filename="FL4SH_FF.txt"):
    """
    Load UID and password from FL4SH_FF.txt file
    """
    try:
        if not os.path.exists(filename):
...
def send_likes(uid):
    try:
        likes_api_response = requests.get(
             f"https://your-like-api.onrender.com/like?uid={uid}&server_name=ind&key=Flash",
             timeout=30
             )
